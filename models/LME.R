...
subjecttrain = rep(1:100,5)

data.train = data.frame(y = c(ytrain), x1 = c(t(X1train)), x2 = c(t(X2train)), 
					x3 = c(t(X3train)), subject = subjecttrain)
f = lme(fixed = y~x1+x2+x3,
	random = ~ 1 | subject, data = data.train )
fhat = matrix(predict(f),5,n)

ytest = y[,6:55]
X1test = X1[,6:55]; X2test = X2[,6:55]; X3test = X3[,6:55]; 
subjecttest = rep(1:100,50)

data.test = data.frame(y = c(t(ytest)), x1 = c(t(X1test)), 
				x2 = c(t(X2test)), x3 = c(t(X3test)), subject = subjecttest)
